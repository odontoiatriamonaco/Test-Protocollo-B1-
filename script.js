...
// NOTA: Il valore HU nei PDF RealGuide è in ROSSO, non ciano!
async function estraiHUConOCRMirato(pdf, pagineConDensita) {
    ...
    for (var idx = 0; idx < pagineConDensita.length; idx++) {
        ...
        // SCALA ALTA per migliore OCR (equivalente ~600 DPI)
        var scale = 6.0;
        ...
        // ZONA ESATTA del valore HU (ottimizzata da test):
        // Il valore ROSSO è sotto il grafico colorato, circa X 27%, Y 55%
        var cropX = Math.floor(viewport.width * 0.27);
        var cropY = Math.floor(viewport.height * 0.55);
        var cropW = Math.floor(viewport.width * 0.10);
        var cropH = Math.floor(viewport.height * 0.05);
        ...
        // FILTRO per ROSSO (il valore HU è in rosso, non ciano!)
        try {
            ...
            for (var px = 0; px < data.length; px += 4) {
                ...
                // ROSSO/ARANCIONE: R alto, G e B bassi
                var isRosso = (r > 150 && r > g * 1.3 && r > b * 1.3);
                
                // Anche testo nero
                var luminosita = 0.299 * r + 0.587 * g + 0.114 * b;
                var isScuro = (luminosita < 80);
        ...
        try {
            ...
            console.log("OCR HU pagina " + pageNum + ":", ocrText);
            
            // Cerca numeri a 3 cifre validi per HU
            // Accetta anche 4 cifre e prende le prime 3 (per errori OCR tipo "4329" -> "432")
            ...
            if (numeri) {
                ...
                for (var n = 0; n < numeri.length; n++) {
                    var numStr = numeri[n].substring(0, 3); // Prendi solo prime 3 cifre
                    var val = parseInt(numStr);
                    // HU tipico per osso: 200-900
