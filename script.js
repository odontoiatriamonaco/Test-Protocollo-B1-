...
function estraiDatiDaReport(testo) {
    ...
    var codiciDimensioni = {
        ...
        '210428': { diametro: '4.2', lunghezza: '8' },   // Codice alternativo per 4.2x8
    ...
    // Usiamo [\s\S] per catturare anche newline tra tipo e codice
    // Il codice può avere spazi (es. "21 0372M")
    var patternCodice = /(\d{2})\s+IDI\s*Evolution\s+B1\s*One\s*[-–—]\s*(High|Medium|Low)[\s\S]*?\[(\d{2})\s*(\d{4})[HML]?\]/gi;
    var matchCodice;
    ...
    while ((matchCodice = patternCodice.exec(testoNorm)) !== null) {
        ...
        if (dente >= 11 && dente <= 48) {
            ...
            var codice = matchCodice[3] + matchCodice[4]; // Ricostruisci codice senza spazi
    ...
    // METODO AGGIUNTIVO: Cerca codici prodotto nelle pagine di dettaglio
    // Ogni pagina di dettaglio ha: Posizione XX ... [codice]
    var pagineDett2 = testo.split(/---\s*PAGINA\s*\d+\s*---/);
    for (var pd = 0; pd < pagineDett2.length; pd++) {
        var pag = pagineDett2[pd];
        
        // Cerca il numero del dente con vari pattern (in ordine di affidabilità)
        var dentePag = null;
        
        // Pattern 1: "Posizione 32" o "Numero impianto: 45" (più affidabile)
        var matchPos = pag.match(/(?:Posizione|Numero\s*impianto)[:\s]*(\d{2})/i);
        if (matchPos) {
            var d = parseInt(matchPos[1]);
            if (d >= 11 && d <= 48) dentePag = d;
        }
        
        // Pattern 2: "XX IDI Evolution" (molto affidabile)
        if (!dentePag) {
            var matchIDI = pag.match(/(\d{2})\s+IDI\s*Evolution/i);
            if (matchIDI) {
                var d = parseInt(matchIDI[1]);
                if (d >= 11 && d <= 48) dentePag = d;
            }
        }
        
        // Pattern 3: "3 5" o "4 2" (OCR con spazio tra le cifre) - solo se nel range valido
        if (!dentePag) {
            var matchSpazio = pag.match(/(\d)\s+(\d)\s+Data/i);
            if (matchSpazio) {
                var d = parseInt(matchSpazio[1] + matchSpazio[2]);
                if (d >= 11 && d <= 48) dentePag = d;
            }
        }
        
        // Pattern 4: "4. 5" o "3. 5" (OCR con punto) - solo se nel range valido
        if (!dentePag) {
            var matchPunto = pag.match(/(\d)[\.\,]\s*(\d)\s+Data/i);
            if (matchPunto) {
                var d = parseInt(matchPunto[1] + matchPunto[2]);
                if (d >= 11 && d <= 48) dentePag = d;
            }
        }
        
        if (!dentePag) continue;
        
        // Cerca il codice prodotto in questa pagina (gestisce spazi nel codice es. "21 0372M")
        var matchCod = pag.match(/\[(\d{2})\s*(\d{4})[HML]?\]/);
        if (!matchCod) continue;
        
        // Ricostruisci il codice senza spazi
        var codicePag = matchCod[1] + matchCod[2];
        var dimPag = codiciDimensioni[codicePag];
        if (!dimPag) continue;
        
        // Cerca il tipo (Low/Medium/High)
        var matchTipoPag = pag.match(/B1\s*One\s*[-–—]\s*(High|Medium|Low)/i);
        var tipoPag = matchTipoPag ? matchTipoPag[1] : 'Medium';
        
        // Aggiorna o aggiungi l'impianto
        var impIndex = -1;
        for (var ii = 0; ii < risultato.impianti.length; ii++) {
            if (risultato.impianti[ii].dente === dentePag) {
                impIndex = ii;
                break;
            }
        }
        ...
        if (impIndex >= 0) {
            // Aggiorna SEMPRE le dimensioni con quelle dalle pagine di dettaglio (più affidabili)
            risultato.impianti[impIndex].diametro = dimPag.diametro;
            risultato.impianti[impIndex].lunghezza = dimPag.lunghezza;
            // Aggiorna anche il tipo se trovato
            if (matchTipoPag) {
                risultato.impianti[impIndex].tipo = tipoPag;
            }
        } else {
            ...
            risultato.impianti.push({
                dente: dentePag,
                tipo: tipoPag,
                diametro: dimPag.diametro,
                lunghezza: dimPag.lunghezza,
                densitaHU: "",
                caricoImmediato: false,
                postEstrattivo: false
            });
