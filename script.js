...
function mostraPopupClasse(index) {
    ...
    var classi = [
        { val: 'D1', color: '#ef5350', border: '#d32f2f', label: 'D1', desc: 'Molto denso' },
        { val: 'D2_D1', color: '#ffcdd2', border: '#ef9a9a', label: 'D2→D1', desc: 'Transizione' },
        { val: 'D2', color: '#ffa726', border: '#f57c00', label: 'D2', desc: 'Corticale' },
        { val: 'D3_D2', color: '#ffe0b2', border: '#ffcc80', label: 'D3→D2', desc: 'Transizione' },
        { val: 'D3', color: '#ffee58', border: '#fdd835', label: 'D3', desc: 'Trabecolare' },
        { val: 'D3_D4', color: '#c8e6c9', border: '#a5d6a7', label: 'D3→D4', desc: 'Transizione' },
        { val: 'D4', color: '#66bb6a', border: '#43a047', label: 'D4', desc: 'Poroso' }
    ];
...
function estraiDatiDaReport(testo) {
    ...
    // Funzione per estrarre dente da testo con vari pattern
    function estraiDenteDaPagina(pagina) {
        ...
        // MA escludiamo numeri > 48 come "7 7"
    ...
    // Funzione per estrarre tipo (Low/Medium/High)
    function estraiTipoDaPagina(pagina) {
        var m = pagina.match(/B1\s*One\s*[-–—]?\s*(High|Medium|Low)/i);
        return m ? m[1] : null;
    }
    
    // Funzione per estrarre HU
    function estraiHUDaPagina(pagina) {
        ...
        // Rimuovi scale fisse
        paginaPulita = paginaPulita.replace(/\b(1000|1250|3071|350|850)\b/g, '');
        
        // Cerca numeri a 3 cifre nel range HU tipico
        var numeri = paginaPulita.match(/\b(\d{3})\b/g);
        if (numeri) {
            for (var i = 0; i < numeri.length; i++) {
                var val = parseInt(numeri[i]);
                // Range HU realistico: 150-900
                if (val >= 150 && val <= 900) {
                    return numeri[i];
                }
            }
    ...
    // Funzione per estrarre dimensioni dal testo (fallback)
    function estraiDimensioniDaTesto(pagina) {
        // Pattern: "X.X x XX mm" o "X.X x XX MM"
        var m = pagina.match(/(\d[\.,]\d)\s*x\s*(\d{1,2})\s*mm/i);
        if (m) {
            var diam = m[1].replace(',', '.');
            var lung = m[2];
            // Valida
            if (['3.2', '3.7', '4.2', '4.8'].indexOf(diam) !== -1) {
                if (['8', '10', '12', '14'].indexOf(lung) !== -1) {
    ...
    for (var p = 0; p < pagine.length; p++) {
        ...
        if (impIndex >= 0) {
            ...
            if (dimensioni) {
                risultato.impianti[impIndex].diametro = dimensioni.diametro;
                risultato.impianti[impIndex].lunghezza = dimensioni.lunghezza;
            }
            if (hu && !risultato.impianti[impIndex].densitaHU) {
                risultato.impianti[impIndex].densitaHU = hu;
            }
            if (tipo) {
                risultato.impianti[impIndex].tipo = tipo;
            }
        } else {
