...
function scaricaProtocolliJSON() {
    var blob = new Blob([JSON.stringify(dbProtocolli, null, 2)], { type: "application/json" });
    var url = URL.createObjectURL(blob);
    var a = document.createElement("a");
    a.href = url;
    a.download = "protocolli_b1one_custom.json";
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    showCustomAlert("File protocolli scaricato! Per renderlo permanente, sostituisci il contenuto di DEFAULT_PROTOCOLS in script.js con il contenuto del file scaricato.");
}

function caricaProtocolliJSON(event) {
    var file = event.target.files[0];
    if (!file) return;
    var reader = new FileReader();
    reader.onload = function(e) {
        try {
            var nuoviProtocolli = JSON.parse(e.target.result);
            if (!nuoviProtocolli["High"]) {
                showCustomAlert("File non valido.");
                return;
            }
            dbProtocolli = nuoviProtocolli;
            localStorage.setItem("b1one_protocols_custom", JSON.stringify(dbProtocolli));
            renderEditorTable();
            showCustomAlert("Protocolli caricati con successo!");
        } catch (err) {
            showCustomAlert("Errore nel file JSON.");
        }
    };
    reader.readAsText(file);
}
