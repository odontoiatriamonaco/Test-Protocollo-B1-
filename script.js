...
// OCR MIRATO: estrae solo i valori HU dalla zona ESATTA del grafico
async function estraiHUConOCRMirato(pdf, pagineConDensita) {
    ...
    for (var idx = 0; idx < pagineConDensita.length; idx++) {
        ...
        // ZONA ESATTA del valore HU:
        // Il valore è SOTTO il grafico colorato D4-D3-D2-D1, alla punta della freccia ciano
        // Posizione verificata: X 20-40%, Y 52-62% della pagina
        var cropX = Math.floor(viewport.width * 0.20);
        var cropY = Math.floor(viewport.height * 0.52);
        var cropW = Math.floor(viewport.width * 0.20);  // 20% larghezza
        var cropH = Math.floor(viewport.height * 0.10); // 10% altezza
        ...
        // FILTRO SPECIFICO per CIANO del grafico RealGuide
        // Il ciano usato sembra essere circa RGB(0, 180-220, 200-230)
        try {
            ...
            for (var px = 0; px < data.length; px += 4) {
                ...
                // Ciano RealGuide: R molto basso, G e B alti e simili
                // Tipicamente: R < 100, G > 150, B > 150, |G-B| < 50
                var isCianoRealguide = (r < 120 && g > 140 && b > 140 && Math.abs(g - b) < 60);
                
                // Anche turchese più scuro
                var isTurchese = (r < 80 && g > 120 && b > 120);
                
                // Testo nero
                var luminosita = 0.299 * r + 0.587 * g + 0.114 * b;
                var isScuro = (luminosita < 60);
                
                if (isCianoRealguide || isTurchese || isScuro) {
                    data[px] = 0;
                    data[px + 1] = 0;
                    data[px + 2] = 0;
                } else {
            ...
            console.log("Pre-processing HU saltato:", e);
        }
        ...
        try {
            ...
            console.log("OCR HU pagina " + pageNum + " (zona precisa):", ocrText);
            
            // Cerca numeri a 3 cifre validi per HU
            var numeri = ocrText.match(/(\d{3})/g);
            if (numeri) {
                // Escludi valori non plausibili
                var esclusi = ['000', '100', '110', '111', '200', '210', '211', '212', '213', '214', 
                               '307', '309', '350', '500', '568', '810', '850', '999', '1000'];
                
                for (var n = 0; n < numeri.length; n++) {
                    var val = parseInt(numeri[n]);
                    // HU tipico per osso: 200-900 (ma può scendere sotto 200 per D4)
                    if (val >= 200 && val <= 900 && esclusi.indexOf(numeri[n]) === -1) {
                        huTrovato = numeri[n];
                        console.log("✓ HU trovato per pagina " + pageNum + ": " + huTrovato);
                        break;
                    }
                }
            }
            
            // Se non trovato, prova OCR senza filtro (immagine originale)
            if (!huTrovato) {
                console.log("Provo OCR senza filtro per pagina " + pageNum);
                var croppedCanvas2 = document.createElement('canvas');
                croppedCanvas2.width = cropW;
                croppedCanvas2.height = cropH;
                var croppedCtx2 = croppedCanvas2.getContext('2d');
                croppedCtx2.drawImage(canvas, cropX, cropY, cropW, cropH, 0, 0, cropW, cropH);
                
                var imageData2 = croppedCanvas2.toDataURL('image/png');
                var result2 = await Tesseract.recognize(imageData2, 'eng', {
                    tessedit_char_whitelist: '0123456789',
                    logger: function(m) {}
                });
                
                var ocrText2 = result2.data.text.replace(/\s/g, '');
                console.log("OCR HU pagina " + pageNum + " (senza filtro):", ocrText2);
                
                var numeri2 = ocrText2.match(/(\d{3})/g);
                if (numeri2) {
                    for (var n2 = 0; n2 < numeri2.length; n2++) {
                        var val2 = parseInt(numeri2[n2]);
                        if (val2 >= 200 && val2 <= 900 && esclusi.indexOf(numeri2[n2]) === -1) {
                            huTrovato = numeri2[n2];
                            console.log("✓ HU trovato (senza filtro) per pagina " + pageNum + ": " + huTrovato);
                            break;
                        }
                    }
                }
            ...
            console.log("Errore OCR HU pagina " + pageNum + ":", ocrErr);
        }
...
function estraiDatiDaReport(testo) {
    ...
    function estraiHUDaPagina(pagina) {
        ...
        var matchOCR = pagina.match(/HU_VALUE_EXTRACTED:\s*(\d{3})/);
        if (matchOCR) {
            return matchOCR[1];
        }
        ...
        // Pattern 1: Cerca numero a 3 cifre DOPO "D4" (tipica posizione del valore HU nel grafico)
        // Il valore HU appare sotto il grafico, spesso dopo le classi D4 D3 D2 D1
        var matchDopoD4 = paginaPulita.match(/D4[\s\S]{0,50}?D[1-3][\s\S]{0,50}?(\d{3})/);
        if (matchDopoD4) {
            var val = matchDopoD4[1];
            var valNum = parseInt(val);
            if (scaleFisse.indexOf(val) === -1 && altriEsclusi.indexOf(val) === -1 && valNum >= 150 && valNum <= 800) {
                return val;
            }
        }
        
        // Pattern 2: Cerca numero isolato a 3 cifre nella zona del grafico densità
        var matchDensita = paginaPulita.match(/[Dd]ensit[àa][\s\S]{0,200}?[D][1-4][\s\S]{0,100}?(\d{3})/);
        if (matchDensita) {
        ...
        if (zonaHU) {
            // Cerca numeri a 3 cifre nella zona
            var numeriZona = zonaHU[0].match(/\b(\d{3})\b/g);
            if (numeriZona) {
        ...
        // Pattern 4: Cerca numero a 3 cifre vicino a indicatore di classe
        var matchClassi = paginaPulita.match(/(\d{3})\s*[^\d]{0,20}?[D][1-4]/);
        if (matchClassi) {
        ...
        var numeriFinali = ultimaParte.match(/\b(\d{3})\b/g);
        if (numeriFinali) {
    ...
    function estraiDimensioniDaTesto(pagina) {
        ...
        if (m1) {
            ...
            if (['3.2', '3.7', '4.2', '4.8', '5.5'].indexOf(diam) !== -1) {
        ...
        // Pattern 2: OCR senza punto "42x12" o "48x10" → interpreta come 4.2x12 o 4.8x10
        var m2 = pagina.match(/\b([345][278])\s*[xX×]\s*(\d{1,2})\s*mm/i);
        if (m2) {
            var diamRaw = m2[1];
            var lung = m2[2];
            // Converte: 32→3.2, 37→3.7, 42→4.2, 48→4.8, 55→5.5
        ...
        var m3 = pagina.match(/\b([345])[\s\.]*([278])\s*[xXiI×]+\s*(\d{1,2})\s*mm/i);
        if (m3) {
            var diam = m3[1] + '.' + m3[2];
            var lung = m3[3];
            if (['3.2', '3.7', '4.2', '4.8', '5.5'].indexOf(diam) !== -1) {
        ...
        if (m4) {
            ...
            if (['3.2', '3.7', '4.2', '4.8', '5.5'].indexOf(diamRaw) !== -1) {
                if (['6', '8', '10', '12', '14', '15', '17'].indexOf(lung) !== -1) {
                    if (lung === '15') lung = '15.5';
...
function mostraSintesiPreparazione() {
    try {
        ...
        function createToothHTML(toothNum, data) {
            var bgStyle = data ? 'background:' + data.bgColor + '; border:3px solid #333;' : 'background:#f0f0f0; border:1px solid #ddd;';
